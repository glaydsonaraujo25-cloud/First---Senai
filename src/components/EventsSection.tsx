  ...
  return (
    <section id="eventos" className={`py-16 sm:py-24 border-t transition-colors ${isDark ? 'bg-slate-950 text-white border-slate-800' : 'bg-white text-slate-900 border-slate-200'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            return (
              <article key={item.program} className={`rounded-3xl border-2 p-6 flex flex-col ${style.border} ${isDark ? 'bg-slate-900' : 'bg-slate-50 shadow-sm'}`}>
                <div className="flex items-center justify-between mb-5">
                  ...
                  {milestoneStatus(item.isoDate) === 'Data prevista já passou' ? <CheckCircle2 className={`w-5 h-5 ${style.icon}`} /> : <Clock3 className={`w-5 h-5 ${style.icon}`} />}
                </div>
                <h3 className="text-xl font-black mb-2">{item.name}</h3>
                <p className={`flex items-center gap-2 text-sm font-bold mb-4 ${style.icon}`}><Calendar className="w-4 h-4" /> {formatSeasonDate(item.isoDate)}</p>
                ...
                <div className={`mt-5 pt-4 border-t ${isDark ? 'border-slate-800' : 'border-slate-200'}`}>
                  <p className="text-xs uppercase tracking-wider text-slate-500 font-bold mb-3">{milestoneStatus(item.isoDate)}</p>
                  <a href={seasonSource} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-sm font-bold text-blue-600 hover:text-blue-500">Fonte oficial <ExternalLink className="w-4 h-4" /></a>
                </div>
              </article>
            );
          })}
        ...
        <a href="/eventos" className="inline-flex mt-6 font-bold text-blue-700 dark:text-blue-300 underline">Ver calendário e jornada da temporada</a>
        ...
        <div className={`mt-5 rounded-2xl border p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
          <div><h3 className="font-black">Quer participar no DF?</h3><p className={`text-sm mt-1 ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>Conheça os caminhos de participação e confirme oportunidades diretamente nos canais oficiais.</p></div>
          <button type="button" onClick={() => onOpenParticipation()} className="min-h-11 px-5 rounded-xl bg-red-600 hover:bg-red-500 text-white font-bold text-sm shrink-0">Como participar</button>
        </div>
      </div>
    </section>
  );
