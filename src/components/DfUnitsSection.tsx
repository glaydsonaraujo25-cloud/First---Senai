...
export const DfUnitsSection: React.FC<DfUnitsSectionProps> = ({ onOpenParticipation }) => {
  const { isDark } = useTheme();
  const card = isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-sm';

  return (
    <section id="unidades-df" className={`py-16 sm:py-24 border-t transition-colors ${isDark ? 'bg-slate-900 text-white border-slate-800' : 'bg-slate-50 text-slate-900 border-slate-200'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-10">
          <div className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border text-xs font-bold mb-4 ${isDark ? 'bg-blue-500/10 border-blue-500/30 text-blue-300' : 'bg-blue-50 border-blue-200 text-blue-700'}`}><MapPin className="w-3.5 h-3.5" /> SENAI-DF PERTO DE VOCÊ</div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-10">
          {units.map(unit => (
            <article key={unit.name} className={`rounded-2xl border p-5 ${card}`}>
              <div className="w-10 h-10 rounded-xl bg-blue-700 text-white flex items-center justify-center mb-4"><School className="w-5 h-5" /></div>
        ...
        <div className="grid lg:grid-cols-12 gap-5">
          <div className={`lg:col-span-5 rounded-3xl border p-6 sm:p-8 ${card}`}>
            <div className="w-12 h-12 rounded-xl bg-blue-700 text-white flex items-center justify-center mb-5"><Building2 className="w-6 h-6" /></div>
            ...
            <div className={`rounded-xl border p-4 mb-5 ${isDark ? 'bg-slate-950 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
              ...
              <p className="font-black flex items-center gap-2"><Phone className="w-4 h-4 text-orange-500" /> (61) 4042-6565</p>
            </div>
            <a href={SENAI_DF_URL} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-sm font-bold text-blue-700 hover:text-blue-600">Abrir SENAI-DF <ExternalLink className="w-4 h-4" /></a>
          </div>

          <div className="lg:col-span-7 grid sm:grid-cols-2 gap-4">
            <a href={SENAI_DF_EDUCACAO_URL} target="_blank" rel="noreferrer" className={`rounded-2xl border p-6 transition-all hover:-translate-y-0.5 ${card} hover:border-orange-400`}>
              ...
              <span className="inline-flex items-center gap-1.5 text-sm font-bold text-orange-600">Consultar portal <ExternalLink className="w-4 h-4" /></span>
            </a>
            ...
            <button type="button" onClick={() => onOpenParticipation()} className="sm:col-span-2 rounded-2xl bg-gradient-to-r from-blue-700 to-orange-500 p-5 text-white text-left flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 shadow-lg">
