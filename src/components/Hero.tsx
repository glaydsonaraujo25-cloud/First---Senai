            ...
            <div className={`rounded-3xl border p-5 sm:p-6 shadow-xl ${isDark ? 'bg-slate-900/90 border-slate-800' : 'bg-slate-50/95 border-slate-200'}`}>
              ...
              <div className="space-y-3">
                {programs.map(({ code, name, detail, icon: Icon, accent, soft }) => (
                  <a key={code} href="#programas" className={`group flex items-center gap-4 rounded-2xl border p-4 transition-all hover:-translate-y-0.5 ${soft}`}>
                    <div className="w-11 h-11 rounded-xl bg-white/80 dark:bg-slate-950/70 border border-white/70 dark:border-slate-800 flex items-center justify-center shrink-0"><Icon className={`w-5 h-5 ${accent}`} /></div>
                    <div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-3"><span className={`text-xs font-black tracking-wider ${accent}`}>{code}</span><span className={`text-[11px] font-semibold ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{detail}</span></div><p className={`font-bold mt-0.5 ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>{name}</p></div>
